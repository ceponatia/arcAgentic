...
// Moisture Keywords
// ============================================================================
export const MOISTURE_KEYWORDS = {
    dry: 'dry',
    parched: 'dry',
    normal: 'normal',
    damp: 'damp',
 ...
 */
const VERB_SUFFIXES = ['s', 'es', 'ed', 'ing', 'er', 'est'];
/**
 ...
 */
export function containsSensoryKeyword(text, keywords) {
 ...
 */
export function detectSensoryType(text) {
    if (containsSensoryKeyword(text, SCENT_INDICATORS))
        return 'scent';
    if (containsSensoryKeyword(text, TEXTURE_INDICATORS))
        return 'texture';
    if (containsSensoryKeyword(text, VISUAL_INDICATORS))
        return 'visual';
    if (containsSensoryKeyword(text, FLAVOR_INDICATORS))
        return 'flavor';
    if (containsSensoryKeyword(text, SOUND_INDICATORS))
        return 'sound';
    return null;
}
/**
 ...
 */
export function getIntensityWord(intensity) {
    if (intensity >= 0.9)
        return 'overwhelming';
    if (intensity >= 0.8)
        return 'strong';
 ...
 */
export function getIntensityKeywordsInRange(minIntensity, maxIntensity) {
 ...
 */
export function extractIntensity(phrase) {
 ...
 */
export function extractTemperature(phrase) {
    const words = phrase.toLowerCase().split(/\s+/);
 ...
 */
export function extractMoisture(phrase) {
    const words = phrase.toLowerCase().split(/\s+/);
    let moisture = 'normal';
    const cleanedWords = [];
    for (const word of words) {
